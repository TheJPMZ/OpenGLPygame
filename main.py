    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEWHEEL:
                if event.y > 0:
                    if zoom_level <= 100:
                        rend.camPosition.z -= 10 * delta_time
                        zoom_level += 10
                elif event.y < 0:
                    if zoom_level > 0:
                        rend.camPosition.z += 10 * delta_time
            ...
            if event.type == pygame.MOUSEMOTION:
                if event.rel[0] != 0:
                    if x_level <= 350 and event.rel[0] >= 0 or x_level >= 10 and event.rel[0] <= 0:
                        rend.camPosition.x += 10 * delta_time * (event.rel[0] / abs(event.rel[0]))
                        x_level += 10 * (event.rel[0] / abs(event.rel[0]))
                if event.rel[1] != 0:
                    if y_level <= 350 and event.rel[1] >= 0 or y_level >= 10 and event.rel[1] <= 0:
                        rend.camPosition.y += 10 * delta_time * (event.rel[1] / abs(event.rel[1]))
        ...
        if keys[K_LEFT]:
            if x_level > 0:
                rend.camPosition.x -= 10 * delta_time
                x_level -= 10
            ...
            if y_level <= 360:
                rend.camPosition.y += 10 * delta_time
                y_level += 10
